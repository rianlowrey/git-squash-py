...
class TestGitSquashConfig:
    ...
    def test_default_config(self):
        ...
        assert config.subject_line_limit == 50
        assert config.body_line_width == 72
        assert config.total_message_limit == 800
        assert config.split_threshold_commits == 20
        assert config.max_retry_attempts == 3
        assert config.default_branch_prefix == "feature/"
    ...
    def test_config_with_overrides(self):
        ...
        assert new_config.body_line_width == 72  # unchanged
    ...
    def test_from_cli_args(self):
        ...
        assert config.default_branch_prefix == "test/"
...
class TestDiffAnalyzer:
    ...
    def test_categorize_commits(self):
        ...
        commits = [
            CommitInfo("hash1", "2025-01-01", "Add new feature", "user", "user@example.com", datetime.now()),
            CommitInfo("hash2", "2025-01-01", "Fix critical bug", "user", "user@example.com", datetime.now()),
            CommitInfo("hash3", "2025-01-01", "Update tests", "user", "user@example.com", datetime.now()),
            CommitInfo("hash4", "2025-01-01", "Optimize performance", "user", "user@example.com", datetime.now()),
        ]
    ...
    def test_detect_special_conditions(self):
        ...
        commits = [
            CommitInfo("h1", "2025-01-01", "Add critical security fix", "u", "u@e.com", datetime.now()),
            CommitInfo("h2", "2025-01-01", "Use mock API client", "u", "u@e.com", datetime.now()),
        ]
...
- NOTE: This is an important note"""
        ...
        body_lines = lines[2:]
        assert all(len(line) <= self.config.body_line_width for line in body_lines if line)
...
class TestMockAIClient:
    ...
    def test_generate_summary_features(self):
        ...
        summary = self.client.generate_summary(
            date="2025-01-01",
            analysis=analysis,
            commit_subjects=["Add cache layer", "Add error handling", "Fix memory leak"]
        )
        
        lines = summary.split('\n')
        subject = lines[0]
        
        assert "Add" in subject
        assert "features" in subject.lower() or "fix" in subject.lower()
        assert len(subject) <= self.config.subject_line_limit
    ...
    def test_suggest_branch_name(self):
        ...
        branch_name = self.client.suggest_branch_name(summaries)
...
class TestGitSquashTool:
    ...
    def setup_method(self):
        ...
        mock_commits = {
            "2025-01-01": [
                CommitInfo("hash1", "2025-01-01T10:00:00", "Add feature A", "user", "user@example.com", base_date),
                CommitInfo("hash2", "2025-01-01T11:00:00", "Fix bug B", "user", "user@example.com", base_date + timedelta(hours=1)),
            ],
            "2025-01-02": [
                CommitInfo("hash3", "2025-01-02T09:00:00", "Add feature C", "user", "user@example.com", base_date + timedelta(days=1)),
            ]
        }
    ...
    def test_prepare_squash_plan_basic(self):
        """Test basic squash plan preparation."""
        plan = self.tool.prepare_squash_plan()
    ...
    def test_prepare_squash_plan_with_date_filter(self):
        """Test squash plan with date filtering."""
        plan = self.tool.prepare_squash_plan(start_date="2025-01-02")
        
        assert len(plan.items) == 1
        assert plan.items[0].date == "2025-01-02"
        assert plan.total_original_commits == 1
    
    def test_prepare_squash_plan_no_commits(self):
        ...
        with pytest.raises(NoCommitsFoundError):
            tool.prepare_squash_plan()
    
    def test_prepare_squash_plan_invalid_date_range(self):
        """Test squash plan with invalid date range."""
        from git_squash.core.types import InvalidDateRangeError
        
        with pytest.raises(InvalidDateRangeError):
            self.tool.prepare_squash_plan(start_date="2025-01-10")
    
    def test_suggest_branch_name(self):
        """Test branch name suggestion."""
        plan = self.tool.prepare_squash_plan()
        branch_name = self.tool.suggest_branch_name(plan)
        
        assert isinstance(branch_name, str)
        assert branch_name.startswith(self.config.default_branch_prefix)
        assert len(branch_name) > len(self.config.default_branch_prefix)
    
    def test_execute_squash_plan(self):
...
class TestErrorHandling:
    ...
    def test_ai_client_fallback(self):
        ...
        try:
            result = failing_client.generate_summary("2025-01-01", analysis, ["test commit"])
